...
description: 将 AI CLI 工具（Codex CLI、Claude Code CLI、GitHub Copilot CLI）的配置同步到 Git 仓库（GitHub/Gitee）。支持跨机器一键同步 AGENTS.md、CLAUDE.md、Skills、MCP 配置等，自动过滤 API Token 等敏感字段。Use this skill when user mentions syncing CLI configs, setup config sync, push/pull configs, or auto sync.
---
...
echo "✅ Setup 完成，接下来生成辅助脚本并执行首次推送..."
```

生成辅助脚本并立即执行首次推送（见下方 Push 工作流）。
...
# ── Codex CLI ──────────────────────────────────────────────────────────────────
if [ -d "$CODEX_DIR" ] && [ -d "$REPO/codex" ]; then
...
| 首次 push 失败（无 upstream）| 改用 `git push --set-upstream origin main` |
| merge 冲突 | `cd ~/.cli-sync-repo && git pull --rebase` 后手动解决 |
| `jq` 未安装 | `sudo apt install jq` 或 `brew install jq` |
| `rsync` 未安装 | `sudo apt install rsync`；已提供 cp 兜底方案 |
...
4. `config.toml` 的 `[projects.*]` 信任路径含本机绝对路径，新机器 pull 后请检查
